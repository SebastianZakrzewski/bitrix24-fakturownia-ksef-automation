...
| Custom Fakturownia invoice numbering with type-specific prefixes (`FULL` `{n}/{MM}/{YYYY}`, `ADVANCE` `Z{n}/{MM}/{YYYY}`, `FINAL` `ZK{n}/{MM}/{YYYY}`) | Globally unique numbers across invoice types; bootstrap ENV for May 2026; auto-numbering disabled in Fakturownia | High |
